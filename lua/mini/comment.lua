...
MiniComment.textobject = function()
  ...
  local parts = H.make_comment_parts({ vim.fn.line('.'), vim.fn.col('.') }, config.options)
...
H.make_comment_parts = function(ref_position, options)
  ...
  -- Structure of 'commentstring': <left> <%s> <right>
  local left, right = unpack(vim.split(cs, '%%s'))
...
H.make_uncomment_function = function(parts)
  ...
  local blank_regex = '^(%s*)' .. vim.trim(l_esc) .. '(.-)' .. vim.trim(r_esc) .. '(%s-)$'
